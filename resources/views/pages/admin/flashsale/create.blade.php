...
<form action="{{ route('admin.flashsale.store') }}" method="post" enctype="multipart/form-data" id="formData">
@csrf              
        ...
        <div class="card">
            ...
            <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-control" placeholder="description in here ..."></textarea>
                    </div>
                    <div class="form-group">
                        <label for="">Start Date Flashsale</label>
                        <input type="datetime-local" name="start_flashsale" class="form-control" placeholder="Start Date in here ...">
                        <span class="text-danger error-text start_flashsale_error" style="font-size:13px"></span>
                            
                    </div>
                    <div class="form-group">
                        <label for="">End Date Flashsale</label>
                        <input type="datetime-local" name="end_flashsale" class="form-control" placeholder="End Date in here ...">
                        <span class="text-danger error-text end_flashsale_error" style="font-size:13px"></span>
                        
                    </div>
                    <div class="form-group">
                        <button type="button" class="btn btn-success" data-toggle="modal" data-target="#modal-product"><i class="mdi mdi-search-web"></i> Search Product Put to Flashsale</button>
                    </div>

                    <hr>
                    <button id="btn-create" type="submit" class="btn btn-block btn-primary"><i class="mdi mdi-send"></i> Save Change</button>
            <div>
        </div>
    </div>
    </div>
...
function showData(){
    $.ajax({
        url : '{{ route("admin.flashsale.create") }}',
        type: 'GET',  
         beforeSend:function(){
            ...
            for(let i = 0; i < response.products.length;i++){
                ...
                <div class="col-md-4 group" id="active-checkbox">
                    ...
                    <div class="card d-inline ml-2" style="width: 10rem;box-shadow:none">
                        ...
                        <input type="number" name="discount[]" placeholder="discount ...." class="form-control discount d-none invisible" value="" data-value=""  
...
if(test.closest('.group').find('.check').is(':checked') == false){
    ...
    test.closest('.group').find('input[type=number]').attr('value','0');
...
$('#formData').submit(function(e) {
    ...
    $.ajax({
        url : '{{ route("admin.flashsale.searchCreate.product") }}',
        type: 'POST',
        data : {
